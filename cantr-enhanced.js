...
// @include       cantr.net/*
// @include       http://test.cantr-mmorpg.pl/*
// @grant         none
...
version 2.6.0
 - links for quick access to "notes" and "items and resources" categories in inventory
...
/*
  ...
  // other
	...
	function listObjects(){
		...
		var mainNode = node.getElementsByClassName('page-left')[1];
	...
	function groupObjects(){
		var node = document.body;
		var centerNode = node.getElementsByClassName('page-left')[0];
	...
	////// POCZĄTEK LINKÓW DO KATEGORII INWENTARZA
	//////


	function linksToInventoryCategories() {
		var navPanel = document.getElementById("navigationPanel");
		var children = navPanel.getElementsByTagName("form");
		
		navPanel.removeChild(children[1]);

		function createFormNode(inputText, categoryId) {

			var newNode = document.createElement("form");
			newNode.method = "post";
			newNode.action = "index.php?page=char.inventory&show_items=" +
				categoryId + "&character=" + getCharId();

			var inputNode = document.createElement("input");
			inputNode.className = "button_charmenu";
		...
		var itemsNode = createFormNode("Items", 2);
		var notesNode = createFormNode("Notes", 1);

		navPanel.insertBefore(notesNode, children[0].nextSibling);
		navPanel.insertBefore(document.createTextNode(" "), children[0].nextSibling); // empty space
		navPanel.insertBefore(itemsNode, children[0].nextSibling);
		
	}

	//////
	////// KONIEC LINKÓW DO KATEGORII INWENTARZA
...
if ((strona == 'building_vehicles' || strona == 'inside_building')
		&& enabledInStorage('BookmarkBuildingsVehicles')){
...
if (document.getElementById('navigationPanel')
		&& enabledInStorage('LinksToInventoryCategories')) {
	linksToInventoryCategories();
}
